...
public class UV implements Comparable<UV> { // Unit Vector
	...
	public UV convert(U to) throws UnitMismatchException {
		if (!unit.isSameQuantity(to)) {
			throw new UnitMismatchException(String.format("%s can not be converted to %s because they are not the same quantity.",
					unit, to));
		}
        UV result = new UV((this.value * unit.getLength()) / to.getLength(), to);
        //System.out.println("Converting " + unit + " to " + to);
        return result;
    }
	...
	@Override
	public int compareTo(UV arg0) {
		try {
			arg0 = arg0.convert(unit); // Just to throw exception
		} catch (UnitMismatchException e) {
			System.err.format("[ERROR] %s and %s could not be compared because they are not the same quantity.", unit(), arg0.unit());
			e.printStackTrace();
			return 0;
		}
		if (value() < arg0.value() - maxError) return -1;
		else if (value() > arg0.value() + maxError) return 1;
		else return 0;
	}
